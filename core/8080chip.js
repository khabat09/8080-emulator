import OPCODE_SIZES from "/core/OPCODE_SIZES.js";
import OPCODE_CYCLES from "/core/OPCODE_CYCLES.js";

class Machine {
	constructor() {
		this.LOG = "";
		this.loadOfset = 0x000;
		this.memory = new Uint8Array(1024 * 65);
		this.SP = 0xffff;
		this.PC = this.loadOfset;
		
		//machine data stuff goes here(probably)
		this.isRomLoaded = false;
		this.inst = new Instruction();
		this.status = 1;
		this.terminal = document.querySelector("#console");
		this.debugMode = 0;
		this.debugMemory = 0;
		this.isInteruptEnabled = 0;
		this.cycles = 0;
		this.cyclesPerSecond = 2000000;
		this.cyclesPerFrame = this.cyclesPerSecond / 60;
		this.cyclesPerhalfFrame = this.cyclesPerFrame / 2;
		this.VRAM = [0x2400, 0x3fff];
		this.shiftRegister = 0;

		//REGISTERS
		this.A = 0;
		this.B = 0;
		this.C = 0;
		this.D = 0;
		this.E = 0;
		this.H = 0;
		this.L = 0;
		
		//16 bit registers
		this.BC = 0;
		this.DE = 0;
		this.HL = 0;
		
		//FLAGS
		this.S = 0;
		this.Z = 0;
		this.P = 0;
		this.CY = 0;
		this.AC = 0;
		
		this.loadRom("/roms/invaders.rom");
	}
	async loadRom(path) {
		this.isRomLoaded = false;
		const res = await fetch(path);
		const data = await res.arrayBuffer();
		const bytes = new Uint8Array(data);
		
		for (let i = 0; i < bytes.length; i++) {
			this.memory[i + this.loadOfset] = bytes[i];
		}
		// if (this.dumpMemory) {
			let memCpy = [...this.memory];
			// console.log(memCpy.slice(0,100));
			
			// memCpy = memCpy.map((i, x)=> x ? i +": 0x" + x.toString(16) : i + ": 0x00")
			// console.log(JSON.stringify(memCpy).slice(0,100));
		// }
		this.isRomLoaded = true;
	}
	
	parity(num) {
		const len = num.toString(2).replaceAll("0", "").length;
		if (len % 2 == 0) return 1;
		return 0;
	}
	
	printChar() {
		const char = String.fromCharCode(this.E);
		this.terminal.textContent += char;
		this.PC = (this.memory[this.SP + 1] << 8) | this.memory[this.SP];
		this.SP += 2;
		console.log("printed char: ", char);
		console.log("Yoooooooooooooooooooooo")
	}
	printString() {
		const addr = (this.D << 8) | this.E;
		let i = 0;
		let char = this.memory[addr];
		let str = "";
		while (char !== 0x24) {
			i++;
			str += String.fromCharCode(char);
			char = this.memory[addr + i];
		}
		this.terminal.textContent += str + "\n";
		this.PC = (this.memory[this.SP + 1] << 8) | this.memory[this.SP];
		this.SP += 2;
		console.log("printed string: ", str);
	}
	
	call(addr) {
		this.memory[this.SP - 1] = (this.PC & 0xff00) >> 8;
		this.memory[this.SP - 2] = (this.PC & 0x00ff);
		this.SP -= 2;
		this.PC = addr;
	}
	
	return () {
		this.PC = this.memory[this.SP & 0xff] | (this.memory[(this.SP + 1) & 0xff] << 8);
		this.SP += 2;
	}
	
	emulate() {
		if(this.cycles > 2000000){
			this.cycles = 0;
		}

		// RST 1 addr = 1 * 8
		if(this.cycles > this.cyclesPerhalfFrame && this.isInteruptEnabled){
			this.call(0x08);
			this.isInteruptEnabled = 0;
			return;
		}
		// RST 2 addr = 2 * 8
		if(this.cycles > this.cyclesPerFrame && this.isInteruptEnabled){
			this.call(0x10);
			this.isInteruptEnabled = 0;
			console.log([...this.memory].slice(this.VRAM[0], this.VRAM[1]).filter(x=>x?1:0));
			console.log("hi");
			
			
			
			return;
		}
		if (this.isRomLoaded && this.status) this.emulateInst();
	}
	
	emulateInst() {
		this.inst.opcode = this.memory[this.PC] & 0xff;
		this.inst.byte2 = this.memory[this.PC + 1];
		this.inst.byte3 = this.memory[this.PC + 2];
		this.inst.addr = (this.inst.byte3 & 0xff) << 8 | (this.inst.byte2 & 0xff)
		if (!this.inst.setSize()) {
			this.status = 0;
			return 0;
		}
		if (this.PC < this.loadOfset) {
			if (this.PC === 0) {
				this.status = 0;
				console.log("PROGRAM EXITED!")
			}
			else if (this.PC === 0x05) {
				if (this.C === 2) {
					this.printChar();
				} else if (this.C === 9) {
					this.printString();
				}
			}
			return;
		}
		
		this.cycles += OPCODE_CYCLES[this.inst.opcode];
		this.PC += this.inst.size;
		this.PC &= 0xffff;
		
		let debugString = "";
		if (this.debugMode) debugString += `opcode: 0x${this.inst.opcode.toString(16)} `;
		
		switch (this.inst.opcode) {
			case 0x00: {
				if (this.debugMode) debugString += "NOP did nothing, PC: 0x" + this.PC.toString(16);
				break;
			}
			
			case 0x01: {
				if (this.debugMode) debugString += `LXI B, D16, move byte2: 0x${this.inst.byte2.toString(16)} to C: 0x${this.C.toString(16)}, move byte3: 0x${this.inst.byte3.toString(16)} to B: 0x${this.B.toString(16)}`;
				this.C = this.inst.byte2;
				this.B = this.inst.byte3;
				break;
			}
			
			case 0x02: {
				if (this.debugMode) debugString += `STAX B, store A: 0x${this.A.toString(16)} at memory addres BC: 0x${((this.B << 8) | this.C).toString(16)}`;
				const addr = (this.B << 8) | this.C;
				this.memory[addr] = this.A;
				break;
			}

			case 0x05: {
				if(this.debugMode) debugString += `DCR B, B: 0x${this.B.toString(16)} -= 1`;
				const res = new Uint8Array([this.B - 1])[0];

				if(res === 0){
					this.Z = 1;
					if(this.debugMode) debugString += "Z flag is set";
				}else{
					this.Z = 0;
					if(this.debugMode) debugString += "Z flag is unset";
				}

				if(this.debugMode) debugString += " =---= ";

				if(((res >> 7) & 1)===1 ){
					this.S = 1;
					if(this.debugMode) debugString += "S flag is set";
				}else{
					this.S = 0;
					if(this.debugMode) debugString += "S flag is unset";
				}

				if(this.debugMode) debugString += " =---= ";

				const parity = this.parity(res);
				if(parity){
					this.P = 1;
					if(this.debugMode) debugString += "P flag is set";
				}else{
					this.P = 0;
					if(this.debugMode) debugString += "P flag is unset";
				}

				if (this.debugMode) debugString += " =---= ";

				if((this.B & 0x0f) + (1) >= 2 ** 4){
					this.AC = 1;
					if(this.debugMode) debugString += "AC flag is set";
				}else{
					this.AC = 0;
					if(this.debugMode) debugString += "AC flag is unset";
				}

				this.B = res;

				break;
			}
			
			case 0x06: {
				if (this.debugMode) debugString += `MVI B, D8, B: 0x${this.B.toString(16)} = byte2: 0x${this.inst.byte2.toString(16)}`;
				this.B = this.inst.byte2;
				break;
			}

			case 0x09:{
				if(this.debugMode) debugString += `DAD B, HL += BC`;
				const HL = ((this.H & 0xff) << 8 ) | (this.L & 0xff);
				const BC = ((this.B & 0xff) << 8 ) | (this.C & 0xff);
				const res = new Uint8Array([HL + BC])[0];
				if(HL + BC >= 2 ** 16){
					this.CY = 1;
					if(this.debugMode) debugString += "CY flag is set";
				}else{
					this.CY = 0;
					debugString += "CY flag is unset";
				}
				this.H = ((res & 0xff00) >> 8);
				this.L = (res & 0x00ff);
				break;
			}
			
			case 0x0c: {
				if (this.debugMode) debugString += `INR C, C++; all flags are set except CY, `;
				const res = new Uint8Array([this.C + 1])[0];
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if ((this.C & 0x0f) + 1 >= 2 ** 4) {
					this.AC = 1;
					if (this.debugMode) debugString += "AC flag is set";
				} else {
					this.AC = 0;
					if (this.debugMode) debugString += "AC flag is unset";
				}
				
				this.C = res;
				
				break;
			}
			
			case 0x0d:{
				if(this.debugMode) debugString += `DCR C, C -= 1`;
				const res = new Uint8Array([this.C - 1])[0];

				if(this.debugMode) debugString += " =---= ";
				if(res === 0){
					this.Z = 1;
				if(this.debugMode) debugString += "Z flag is set";
					
				}else{
					this.Z = 0;
				if(this.debugMode) debugString += "Z flag is unset";
				}

				if(this.debugMode) debugString += " =---= ";

				if(((res >> 7) & 0xf) === 1){
					this.S = 1;
				if(this.debugMode) debugString += "S flag is set";
				}else{
					this.S = 0;
					if(this.debugMode) debugString += "S flag is unset";
				}

				if(this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if(parity){
					this.P = 1;
					if(this.debugMode) debugString += "P flag is set";
				}else{
					this.P = 0;
					if(this.debugMode) debugString += "P flag is unset";
				}

				if(this.debugMode) debugString += " =---= ";

				if((this.C & 0xf) < 1){
					this.AC = 1;
				if(this.debugMode) debugString += "AC flag is set";
				}else{
					this.AC = 0;
				if(this.debugMode) debugString += "AC flag is unset";
				}

				this.C = res;

				break;
			}

			case 0x0e: {
				if (this.debugMode) debugString += `MVI C, D8, set C: 0x${this.C.toString(16)} to byte2: 0x${this.inst.byte2.toString(16)}`
				this.C = this.inst.byte2;
				break;
			}
			
			case 0x0f: {
				if (this.debugMode) debugString += `RRC, set CY: 0b${this.CY.toString(16)} to bit 0 of A: 0b${this.A.toString(2)} and shift A by 1 to the right and put CY at bit 7 of A, `;
				this.CY = (this.A & 1);
				this.A = ((this.A >> 1) | (this.CY << 7)) & 0xff;
				if (this.debugMode) debugString += `now: CY: 0b${this.CY.toString(2)} and A: 0b${this.A.toString(2)}`;
				break;
			}
			
			case 0x11: {
				if (this.debugMode) debugString += `LXI D, D16, D: 0x${this.D.toString(16)} = byte3: 0x${this.inst.byte3.toString(16)}, E: 0x${this.E.toString(16)} = byte2: : 0x${this.inst.byte2.toString(16)}`;
				this.D = this.inst.byte3;
				this.E = this.inst.byte2;
				break;
			}
			
			case 0x13: {
				if (this.debugMode) debugString += `INX D, DE: 0x${(((this.D & 0x00ff) << 8) | (this.E & 0x00ff)).toString(16)} += 1`;
				const res = (((this.D & 0x00ff) << 8) | (this.E & 0x00ff)) + 1;
				this.D = (res & 0xff00) >> 8;
				this.E = (res & 0x00ff);
				break;
			}

			case 0x19:{
				if(this.debugMode) debugString += `DAD D HL += DE`;
				const HL = ((this.H & 0xff) << 8 ) | (this.L & 0xff);
				const DE = ((this.D & 0xff) << 8 ) | (this.E & 0xff);
				const res = new Uint8Array([HL + DE])[0];
				if(HL + DE >= 2 ** 16){
					this.CY = 1;
					if(this.debugMode) debugString += "CY flag is set";
				}else{
					this.CY = 0;
					if(this.debugMode) debugString += "CY flag is unset";
				}
				break;
			}
			
			case 0x1a: {
				const addr = ((this.D & 0x00ff) << 8) | (this.C & 0x00ff);
				const data = this.memory[addr];
				if (this.debugMode) debugString += `LDAX D, A: 0x${this.A.toString(16)} = (DE: 0x${addr.toString(16)}): 0x${data.toString(16)}`;
				this.A = data;
				
				break;
			}
			
			case 0x1e: {
				if (this.debugMode) debugString += `MVI E,D8, E: 0x${this.E.toString(16)} = byte2: 0x${this.inst.byte2.toString(16)}`
				this.E = this.inst.byte2;
				break;
			}
			
			case 0x21: {
				if (this.debugMode) debugString += `LXI H, D16,  set H: 0x${this.H.toString(16)} to byte3: 0x${this.inst.byte3.toString(16)}, set L: 0x${this.L.toString(16)} to byte2: 0x${this.inst.byte2.toString(16)}`;
				this.H = this.inst.byte3;
				this.L = this.inst.byte2;
				break;
			}
			
			case 0x23: {
				if (this.debugMode) debugString += `INX HL, HL: 0x${(((this.H & 0x00ff) << 8) | (this.L & 0x00ff)).toString(16)} += 1`;
				const res = (((this.H & 0x00ff) << 8) | (this.L & 0x00ff)) + 1;
				this.H = (res & 0xff00) >> 8;
				this.L = (res & 0x00ff);
				break;
			}
			
			case 0x26:{
				if(this.debugMode) debugString += `MVI H,D8, L: 0x${this.L.toString(16)} = byte2: 0x${this.inst.byte2.toString(16)}`;
				this.L = this.inst.byte2;
				break;
			}

			case 0x29:{
				if(this.debugMode) debugString += `DAD H, HL += HL, only set CY flag`;
				const HL = ((this.H & 0xff) << 8) | (this.L & 0xff);
				const res = new Uint8Array([HL + HL])[0];
				if(HL + HL >= 2**16){
					this.CY = 1;
					if(this.debugMode) debugString += "CY flag is set";
				}else{
					this.CY = 0;
					if(this.debugMode) debugString += "CY flag is unset";
				}

				this.H = (HL & 0xff00) << 8;
				this.L = HL & 0x00ff;
				break;
			}

			case 0x2a: {
				const data1 = this.memory[this.inst.addr];
				const data2 = this.memory[this.inst.addr + 1];
				if (this.debugMode) debugString += `LHLD addr, L: 0x${this.L.toString(16)} = (adr: 0x${this.inst.addr.toString(16)}): 0x${data1.toString(16)}, H: 0x${this.H.toString(16)} = ((addr + 1): 0x${(this.inst.addr+1).toString(16)}): 0x${data2.toString(16)}`;
				this.L = data1;
				this.H = data2;
				break;
			}
			
			case 0x31: {
				if (this.debugMode) debugString += `set (SP): 0x${this.memory[this.SP].toString(16)} to addr: 0x${this.inst.addr.toString(16)}, `
				this.SP = this.inst.addr;
				break;
			}
			
			case 0x32: {
				if (this.debugMode) debugString += `STA addr, (addr: 0x${this.inst.addr.toString(16)}): ${this.memory[this.inst.addr].toString(16)} = A: 0x${this.A.toString(16)}`;
				this.memory[this.inst.addr] = this.A;
				break;
			}

			case 0x36: {
				const addr = ((this.H & 0x00ff) >> 8) | (this.L & 0x00ff);
				if(this.debugMode) debugString += `MVI M, D8, (HL: 0x${(addr).toString(16)}): 0x${this.memory[addr].toString(16)} = byte2: 0x${this.inst.byte2.toString(16)}`;
				this.memory[addr] = this.inst.byte2;
				break;
			}
			
			case 0x3a: {
				const data = this.memory[this.inst.addr];
				if (this.debugMode) debugString += `LDA addr, A: 0x${this.A.toString(16)}  = (addr: 0x${this.inst.addr}): 0x${data.toString(16)}`;
				this.A = data;
				break;
			}
			
			case 0x3c: {
				if (this.debugMode) debugString += `INR A, set all flags except CY, A: 0x${this.A.toString(16)} += 1`;
				const res = new Uint8Array([this.A + 1])[0];
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				if ((this.A & 0x0f) + 1 >= 2 ** 4) {
					this.AC = 1;
					if (this.debugMode) debugString += "AC flag is set";
				} else {
					this.AC = 0;
					if (this.debugMode) debugString += "AC flag is unset";
				}
				
				break;
			}
			
			case 0x3e: {
				if (this.debugMode) debugString += `MVI A, D8, A: 0x${this.A.toString(16)} = byte2: 0x${this.inst.byte2.toString(16)}`;
				this.A = this.inst.byte2;
				break;
			}
			
			case 0x52: {
				if (this.debugMode) debugString += "MOV D,D";
				this.D = this.D;
				break;
			}
			
			case 0x5f: {
				if (this.debugMode) debugString += `MOV E, A, E: 0x${this.E.toString(16)} = A: 0x${this.A.toString(16)}`;
				this.E = this.A;
				break;
			}
			
			case 0x6f:{
				if(this.debugMode) debugString += `MOV L,A`;
				this.L = this.A;
				break;
			}

			case 0x77: {
				const addr = ((this.H & 0x00ff) << 8) | (this.L & 0x00ff);
				if (this.debugMode) debugString += `MOV M,A, (HL: 0x${addr.toString(16)}): 0x${this.memory[addr].toString(16)} = A: 0x${this.A.toString(16)}`;
				this.memory[addr] = this.A;
				break;
			}
			
			case 0x7c: {
				if (this.debugMode) debugString += `MV A, H, A: 0x${this.A.toString(16)} = H: 0x${this.H.toString(16)}`;
				this.A = this.H;
				break;
			}
			
			case 0x7d: {
				if (this.debugMode) debugString += `MOV A,L, A: 0x${this.A.toString(16)} = L: 0x${this.L.toString(16)}`;
				this.A = this.L;
				break;
			}
			
			case 0x7e: {
				const addr = (this.H << 8) | this.L;
				const data = this.memory[addr];
				if (this.debugMode) debugString += `MOV A,M, A: 0x${this.A.toString(16)} = (HL: 0x${addr.toString(16)}): 0x${data.toString(16)}`;
				this.A = data;
				break;
			}
			
			case 0xa6: {
				const addr = (this.H << 8) | this.L
				const data = this.memory[addr];
				const res = this.A & data;
				if (this.debugMode) debugString += `ANA M, A: 0x${this.A.toString(16)} = (A: 0x${this.A.toString(16)} & (HL): 0x${data.toString(16)}): 0x${res.toString(16)} `
				
				this.CY = 0;
				
				if (res === 0) {
					this.Z = 1;
				} else {
					this.Z = 0;
				}
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
				} else {
					this.S = 0;
				}
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
				} else {
					this.P = 0;
				}
				
				const ACRes = (((this.A >> 3) & 1) | ((data >> 3) & 1));
				if (ACRes) {
					this.AC = 1;
				} else {
					this.AC = 0;
				}
				
				this.A = res;
				break;
			}
			case 0xac: {
				const res = this.A ^ this.H;
				if (this.debugMode) debugString += `XRA H, A = (A: 0x${this.A.toString(16)} ^ H: 0x${this.H.toString(16)}): 0x${res.toString(16)}`;
				this.CY = 0;
				this.AC = 0;
				
				if (res === 0) {
					this.Z = 1;
				} else {
					this.Z = 0;
				}
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
				} else {
					this.S = 0;
				}
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
				} else {
					this.P = 0;
				}
				
				break;
			}
			
			case 0xaf: {
				if (this.debugMode) debugString += `XRA A, A: 0x${this.A.toString(16)} ^= A, `;
				this.A = 0;
				
				this.CY = 0;
				this.AC = 0;
				this.Z = 1;
				this.S = 0;
				this.P = 1;
				const ACRes = (this.A >> 3) & 1
				
				break;
			}
			
			case 0xc0: {
				if (this.debugMode) debugString += `RNZ, if Z: ${this.Z} is NOT set do pop pc from stack and change PC: 0x${this.PC.toString(16)} to (SP): 0x${this.memory[this.SP].toString(16)} and (SP+1): 0x${this.memory[this.SP+1].toString(16)}`;
				if (this.Z === 0) {
					this.return();
					this.cycles += 6;
				}
				break;
			}

			case 0xc1:{
				if(this.debugMode) debugString += `POP B, C = (SP) and B =(SP+1) and SP += 2`;
				this.C = this.memory[this.SP];
				this.B = this.memory[this.SP + 1];
				this.SP += 2;
				break;
			}
			
			case 0xc2: {
				debugString += `JNZ addr, if flag Z: ${this.Z} is NOT set change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)}, =---= `
				if (!this.Z) {
					this.PC = this.inst.addr;
					debugString += `flag Z: ${this.Z} is set and PC: 0x${this.PC.toString(16)} is set to addr: 0x${this.inst.addr.toString(16)}`;
				} else debugString += `flag Z: ${this.Z} is not set pc: 0x${this.PC.toString(16)} is the same`
				break;
			}
			
			case 0xc3: {
				if (this.debugMode) debugString += `JMP addr, changed PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)}`;
				this.PC = this.inst.addr;
				break;
			}
			
			case 0xc4: {
				if (this.debugMode) debugString += `CNZ addr, if flag NOT Z: ${this.Z} do push pc and change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)} `;
				if (this.Z === 0) {
					this.call(this.inst.addr)
					this.cycles += 6;
				}
				break;
			}
			
			case 0xc5: {
				if (this.debugMode) debugString += `PUSH B, (SP-1) = B: 0x${this.B.toString(16)}, (SP-2) = C: 0x${this.C.toString(16)} and SP -= 2`;
				this.memory[this.SP - 1] = this.B;
				this.memory[this.SP - 2] = this.C;
				this.SP -= 2;
				break;
			}
			
			case 0xc6: {
				if (this.debugMode) debugString += `ADI D8, A += byte2 (A: 0x${this.A.toString(16)} + byte2: 0x${this.inst.byte2.toString(16)}): 0x${(new Uint8Array([this.A + this.inst.byte2])[0]).toString(16)}, and set all flags, `;
				
				const res = new Uint8Array([this.A + this.inst.byte2])[0];
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (this.A + this.inst.byte2 > 255) {
					this.CY = 1;
					if (this.debugMode) debugString += "CY flag is set";
				} else {
					this.CY = 0;
					if (this.debugMode) debugString += "CY flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if ((this.A & 0x0f) + (this.inst.byte2 & 0x0f) >= 2 ** 4) {
					this.AC = 1;
					if (this.debugMode) debugString += "AC flag is set";
				} else {
					this.AC = 0;
					if (this.debugMode) debugString += "AC flag is unset";
				}
				
				
				this.A = res;
				break;
			}
			
			case 0xc8: {
				if (this.debugMode) debugString += `RZ, if flag Z: ${this.Z} is set do RET, put (SP): 0x${this.memory[this.SP & 0xff].toString(16)} to PC.low: 0x${(this.PC & 0x00ff).toString(16)} and put SP+1: 0x${(this.memory[(this.SP+1) & 0xff].toString(16))} to PC.hi: 0x${((this.PC & 0xff00)>>8).toString(16)} and SP += 2, `;
				if (this.Z) {
					this.return();
					if (this.debugMode) debugString += `PC is now: 0x${this.PC.toString(16)}`;
					this.cycles += 6;
				}
				break;
			}
			
			case 0xc9: {
				if (this.debugMode) debugString += `RET, put (SP): 0x${this.memory[this.SP].toString(16)} to PC.low: 0x${(this.PC & 0x00ff).toString(16)} and put SP+1: 0x${(this.memory[this.SP+1].toString(16))} to PC.hi: 0x${((this.PC & 0xff00)>>8).toString(16)} and SP += 2`;
				this.PC = (this.memory[this.SP + 1] << 8) | this.memory[this.SP];
				this.SP += 2;
				if (this.debugMode) debugString += `,  PC: 0x${this.PC.toString(16)}`;
				break;
			}
			
			case 0xca: {
				debugString += `JZ addr, if flag Z: ${this.Z} is set change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)}, =---= `
				if (this.Z) {
					this.PC = this.inst.addr;
					debugString += `flag Z: ${this.Z} is set and PC: 0x${this.PC.toString(16)} is set to addr: 0x${this.inst.addr.toString(16)}`;
				} else debugString += `flag Z: ${this.Z} is not set pc: 0x${this.PC.toString(16)} is the same`
				break;
			}
			
			case 0xcc: {
				if (this.debugMode) debugString += `CZ addr, if flag Z: ${this.Z} do push pc and change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)} =---= `;
				if (this.Z) {
					this.memory[this.SP - 1] = (this.PC & 0xff00) >> 8;
					this.memory[this.SP - 2] = (this.PC & 0xff);
					this.SP -= 2;
					this.PC = this.inst.addr;
					if (this.debugMode) debugString += `did: pushed PC and set PC to addr`;
					this.cycles += 6;
				}
				else if (this.debugMode) debugString += "did: nothing, Z is not set";
				
				break;
			}
			
			case 0xcd: {
				if (this.debugMode) debugString += `CALL adr, put PC.hi: 0x${((this.PC & 0xff00)>>8).toString(16)} into SP-1: 0x${this.memory[this.SP].toString(16)} and PC.lo: 0x${(this.PC & 0x00ff).toString(16)} (in SP-2): 0x${(this.memory[this.SP-1]).toString(16)} and SP=-2 and set PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)} `;
				
				this.memory[this.SP - 1] = ((this.PC & 0xff00) >> 8);
				this.memory[this.SP - 2] = (this.PC & 0xff);
				this.SP -= 2;
				this.PC = this.inst.addr;
				
				break;
			}
			
			case 0xce: {
				if (this.debugMode) debugString += `ACI D8, A += byte2 + CY (A: 0x${this.A.toString(16)} + byte2: 0x${this.inst.byte2} + CY: 0x${this.CY.toString(16)}): 0x${(new Uint8Array([this.A + this.inst.byte2 + this.CY])).toString(16)}, and set all flags, `;
				
				const res = new Uint8Array([this.A + this.inst.byte2 + this.CY])[0];
				let CYRes = 0;
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (this.A + this.inst.byte2 + this.CY > 255) {
					CYRes = 1;
					if (this.debugMode) debugString += "CY flag is set";
				} else {
					CYRes = 0;
					if (this.debugMode) debugString += "CY flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if ((this.A & 0x0f) + (this.inst.byte2 & 0x0f) + (this.CY & 0x0f) >= 2 ** 4) {
					this.AC = 1;
					if (this.debugMode) debugString += "AC flag is set";
				} else {
					this.AC = 0;
					if (this.debugMode) debugString += "AC flag is unset";
				}
				
				
				this.A = res;
				this.CY = CYRes;
				break;
			}
			
			case 0xd0: {
				if (this.debugMode) debugString += `RNC, if CY: ${this.CY} is NOT set do pop pc from stack and change PC: 0x${this.PC.toString(16)} to (SP): 0x${this.memory[this.SP].toString(16)} and (SP+1): 0x${this.memory[this.SP+1].toString(16)}`;
				if (this.CY === 0) {
					this.return();
					this.cycles += 6;
				}
				break;
			}
			
			case 0xd1: {
				if (this.debugMode) debugString += `POP D, put (SP): 0x${this.memory[this.SP].toString(16)} into E: 0x${this.E.toString(16)} and SP+1: 0x${(this.memory[this.SP+1]).toString(16)} into D: 0x${this.D.toString(16)}, SP += 2`;
				this.E = this.memory[this.SP];
				this.D = this.memory[this.SP + 1];
				this.SP += 2;
				break;
			}
			
			case 0xd2: {
				if (this.debugMode) debugString += `JNC addr, jump to addr: 0x${this.inst.addr} if CY: ${this.CY} is NOT set, `;
				if (!this.CY) {
					this.PC = this.inst.addr;
					if (this.debugMode) debugString += "DID jump";
				} else if (this.debugMode) debugString += "DID NOT jump";
				break;
			}
			
			case 0xd5: {
				if (this.debugMode) debugString += `PUSH D, set (SP-2): 0x${(this.memory[this.SP-2]).toString(16)} to E: 0x${this.E.toString(16)}, set (SP-1): 0x${(this.memory[this.SP-1]).toString(16)} to D: 0x${this.D.toString(16)}, sp-= 2`;
				this.memory[this.SP - 1] = this.D;
				this.memory[this.SP - 2] = this.E;
				this.SP -= 2;
				break;
			}
			
			case 0xd3:{
				if(this.debugMode) debugString += `OUT D8, port 2 special, set shift register to byte2`;
				this.shiftRegister = this.inst.byte2;
				break;
			}

			case 0xd4: {
				if (this.debugMode) debugString += `CNC addr, if CY: ${this.CY} is NOT set do push pc and change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)} `;
				if (this.CY === 0) {
					this.memory[this.SP - 1] = (this.PC & 0xff00) >> 8;
					this.memory[this.SP - 2] = (this.PC & 0xff);
					this.SP -= 2;
					this.PC = this.inst.addr;
					this.cycles += 6;
				}
				break;
			}
			
			case 0xd6: {
				const res = new Uint8Array([this.A - this.inst.byte2])[0];
				if (this.debugMode) debugString += `SUI D8, A -= byte2 (A: 0x${this.A.toString(16)} - byte2: 0x${this.inst.byte2.toString(16)}): 0x${res.toString(16)}, all flags are set, `;
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (this.A < this.inst.byte2) {
					this.CY = 1;
					if (this.debugMode) debugString += "CY flag is set";
				} else {
					this.CY = 0;
					if (this.debugMode) debugString += "CY flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if ((this.A & 0x0f) < (this.inst.byte2 & 0x0f)) {
					this.AC = 1;
					if (this.debugMode) debugString += "AC flag is set";
				} else {
					this.AC = 0;
					if (this.debugMode) debugString += "AC flag is unset";
				}
				
				this.A = res;
				break;
			}
			
			case 0xd8: {
				if (this.debugMode) debugString += `RC, if CY: ${this.CY} is set do pop pc from stack and change PC: 0x${this.PC.toString(16)} to (SP): 0x${this.memory[this.SP].toString(16)} and (SP+1): 0x${this.memory[this.SP+1].toString(16)} and SP += 2 `;
				if (this.CY === 1) {
					this.return();
					this.cycles += 6;
				}
				break;
			}
			
			case 0xda: {
				if (this.debugMode) debugString += `JC addr, jump to addr: 0x${this.inst.addr} if CY: ${this.CY} is set, `;
				if (this.CY) {
					this.PC = this.inst.addr;
					if (this.debugMode) debugString += "DID jump";
				} else if (this.debugMode) debugString += "DID NOT jump";
				break;
			}
			
			case 0xdc: {
				if (this.debugMode) debugString += `CC addr, if CY: ${this.CY} call addr: 0x${this.inst.addr.toString(16)}, push PC.hi: 0x${this.PC.toString(16)} to sp-1 and PC.lo: 0x${this.PC.toString(16)} to sp-2 and change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)}`;
				if (this.CY === 1) {
					this.memory[this.SP - 1] = (this.PC & 0xff00) >> 8;
					this.memory[this.SP - 2] = this.PC & 0x00ff;
					this.SP -= 2;
					this.PC = this.inst.addr;
					this.cycles += 6;
				}
				break;
			}
			
			case 0xde: {
				const res = new Uint8Array([this.A - this.inst.byte2 - this.CY])[0];
				if (this.debugMode) debugString += `SBI d8, A = (A: 0x${this.A.toString(16)} - byte2: 0x${this.inst.byte2.toString(16)} - CY: 0x${this.CY.toString(16)}): 0x${res.toString(16)}, and set all flags, `;
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (this.A < this.inst.byte2 + this.CY) {
					this.CY = 1;
					if (this.debugMode) debugString += "CY flag is set";
				} else {
					this.CY = 0;
					if (this.debugMode) debugString += "CY flag is unset";
				}
				if (this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if ((this.A & 0x0f) < ((this.inst.byte2 + this.CY) & 0x0f)) {
					this.AC = 1;
					if (this.debugMode) debugString += "AC flag is set";
				} else {
					this.AC = 0;
					if (this.debugMode) debugString += "AC flag is unset";
				}
				
				this.A = res;
				break;
			}
			
			
			case 0xe0: {
				if (this.debugMode) debugString += `RPO, if P: ${this.P} flag is unset do pop pc from stack and change PC: 0x${this.PC.toString(16)} to (SP): 0x${this.memory[this.SP].toString(16)} and (SP+1): 0x${this.memory[this.SP+1].toString(16)} and SP += 2 `;
				if (this.P === 0) {
					this.return();
					this.cycles += 6;
				}
				break;
			}
			
			case 0xe1: {
				if (this.debugMode) debugString += `POP H, put (SP): 0x${this.memory[this.SP].toString(16)} into L: 0x${this.L.toString(16)} and (SP+1): 0x${(this.memory[this.SP+1]).toString(16)} into H: 0x${this.H.toString(16)}, SP += 2`;
				this.L = this.memory[this.SP];
				this.H = this.memory[this.SP + 1];
				this.SP += 2;
				break;
			}
			
			case 0xe2: {
				if (this.debugMode) debugString += `JPO addr, jump to addr: 0x${this.inst.addr.toString(16)} if P: ${this.P} flag is unset, `;
				if (!this.P) {
					this.PC = this.inst.addr;
					if (this.debugMode) debugString += "DID jump"
				} else if (this.debugMode) debugString += "DID NOT jump"
				break;
			}
			
			case 0xe4: {
				if (this.debugMode) debugString += `CPO addr, if P: ${this.P} flag is unset call addr: 0x${this.inst.addr.toString(16)}, push PC.hi: 0x${this.PC.toString(16)} to sp-1 and PC.lo: 0x${this.PC.toString(16)} to sp-2 and change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)}`;
				if (this.P === 0) {
					this.call(this.inst.addr)
					this.cycles += 6;
				}
				break;
			}
			
			case 0xe5: {
				if (this.debugMode) debugString += `PUSH H, set (SP-2): 0x${(this.memory[this.SP-2]).toString(16)} to L: 0x${this.L.toString(16)}, set (SP-1): 0x${(this.memory[this.SP-1]).toString(16)} to H: 0x${this.H.toString(16)}, sp-= 2`;
				this.memory[this.SP - 1] = this.L;
				this.memory[this.SP - 2] = this.H;
				this.SP -= 2;
				break;
			}
			case 0xe6: {
				if (this.debugMode) debugString += `ANI D8, (A: 0x${this.A} &= byte2: 0x${this.inst.byte2}): 0x${this.A & this.inst.byte2} and all flags are set, `;
				const res = this.A & this.inst.byte2;
				
				if (this.debugMode) debugString += "CY flag is unset";
				this.CY = 0;
				
				debugString += " =---= ";
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				debugString += " =---= ";
				
				if (res >> 7 == 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				debugString += " =---= ";
				
				const ACRes = ((this.A | this.inst.byte2) >> 3) & 1;
				if (ACRes) {
					this.AC = 1;
					if (this.debugMode) debugString += "AC flag is set";
				} else {
					this.AC = 0;
					if (this.debugMode) debugString += "AC flag is unset";
				}
				
				this.A = res;
				break;
			}
			
			case 0xe8: {
				if (this.debugMode) debugString += `RPE, if P: ${this.P} flag is set do pop pc from stack and change PC: 0x${this.PC.toString(16)} to (SP): 0x${this.memory[this.SP].toString(16)} and (SP+1): 0x${this.memory[this.SP+1].toString(16)} and SP += 2 `;
				if (this.P === 1) {
					this.return();
					this.cycles += 6;
				}
				break;
			}
			
			case 0xea: {
				if (this.debugMode) debugString += `JPE addr, jump to addr: 0x${this.inst.addr.toString(16)} if P: ${this.P} flag is set, `;
				if (this.P) {
					this.PC = this.inst.addr;
					if (this.debugMode) debugString += "DID jump"
				} else if (this.debugMode) debugString += "DID NOT jump"
				break;
			}
			
			case 0xeb: {
				if (this.debugMode) debugString += `XCHG, swap D: 0x${this.D.toString(16)} and H: 0x${this.H.toString(16)} and swap E: 0x${this.E.toString(16)} and L: 0x${this.L.toString(16)} `;
				const tmpD = this.D;
				const tmpE = this.E;
				this.D = this.H;
				this.E = this.L;
				this.H = tmpD;
				this.L = tmpE;
				break;
			}
			
			case 0xec: {
				if (this.debugMode) debugString += `CPE addr, if P: ${this.P} flag is set do push pc and change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)} `;
				if (this.P === 1) {
					this.call(this.inst.addr);
					this.cycles += 6;
				}
				break;
			}
			
			case 0xee: {
				const res = this.A ^ this.inst.byte2;
				if (this.debugMode) debugString += `XRI D8, A = (A: 0x${this.A.toString(16)} ^ byte2: 0x${this.inst.byte2.toString(16)}): 0x${res.toString(16)}, and set all flags, `;
				this.CY = 0;
				this.AC = 0;
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				
				this.A = res;
				break;
			}
			
			case 0xf0: {
				if (this.debugMode) debugString += `RP addr, jump to addr: 0x${this.inst.addr.toString(16)} if S: ${this.S} is unset, `;
				if (this.S === 0) {
					this.return();
					this.cycles += 6;
				}
				break;
			}
			
			case 0xf1: {
				if (this.debugMode) debugString += "POP PSW, go look your self im not explaining or debuging im just hoping for the best"
				this.A = this.memory[this.SP + 1];
				const flagByte = this.memory[this.SP];
				
				this.S = (flagByte & 0b10000000) >> 7;
				this.Z = (flagByte & 0b01000000) >> 6;
				this.AC = (flagByte & 0b00010000) >> 4;
				this.P = (flagByte & 0b00000100) >> 2;
				this.CY = flagByte & 0b00000001;
				
				this.SP += 2;
				break;
			}
			
			case 0xf2: {
				if (this.debugMode) debugString += `JP addr, jump to addr: 0x${this.inst.addr.toString(16)} if S: ${this.S} is unset, `;
				if (this.S === 0) {
					this.PC = this.inst.addr;
					if (this.debugMode) debugString += "DID JUMP";
				} else if (this.debugMode) debugString += "DID NOT JUMP";
				break;
			}
			
			case 0xf3: {
				if (this.debugMode) debugString += "DI, disable interupts";
				this.isInteruptEnabled = 0;
				break;
			}
			
			case 0xf4: {
				if (this.debugMode) debugString += `CP addr, if flag P: ${this.P} do push pc and change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)} =---= `;
				if (this.P) {
					this.call(this.inst.addr);
					if (this.debugMode) debugString += `did: pushed PC and set PC to addr`;
					this.cycles += 6;
				}
				if (this.debugMode) debugString += "did: nothing, P is not set";
				break;
			}
			
			case 0xf5: {
				if (this.debugMode) debugString += "PUSH PSW, put A into SP-1 and all flags to SP-2 for mere info check your self";
				this.memory[this.SP - 1] = this.A;
				this.memory[this.SP - 2] = (
					(this.S << 7) |
					(this.Z << 6) |
					(0 << 5) |
					(this.AC << 4) |
					(0 << 3) |
					(this.P << 2) |
					(1 << 1) |
					this.CY
				);
				this.SP -= 2;
				break;
			}
			
			case 0xf6: {
				const res = this.A | this.inst.byte2;
				if (this.debugMode) debugString += `ORI D8, A = (A: 0x${this.A.toString(16)} | byte2: 0x${this.inst.byte2.toString(16)}): 0x${res.toString(16)}, and set all flags, `;
				
				this.CY = 0;
				this.AC = 0;
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += "Z flag is set";
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += "Z flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (((res >> 7) & 1) === 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set";
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				this.A = res;
				
				break;
			}
			
			
			case 0xf8: {
				if (this.debugMode) debugString += `RM addr, jump to addr: 0x${this.inst.addr.toString(16)} if S: ${this.S} is set, `;
				if (this.S === 1) {
					this.return();
					this.cycles += 6;
				}
				break;
			}
			
			case 0xf9: {
				const HL = (this.H << 8) | this.L;
				if (this.debugMode) debugString += `SPHL, (SP): 0x${this.memory[this.SP].toString(16)} = HL: 0x${HL.toString(16)}`;
				this.SP = HL;
				break;
			}
			
			case 0xfa: {
				if (this.debugMode) debugString += `JM addr, jump to addr: 0x${this.inst.addr.toString(16)} if S: ${this.S} is set, `;
				if (this.S === 1) {
					this.PC = this.inst.addr;
					if (this.debugMode) debugString += "DID JUMP";
				} else if (this.debugMode) debugString += "DID NOT JUMP";
				break;
			}
			
			case 0xfb: {
				if (this.debugMode) debugString += "EI, Enable interupts";
				this.isInteruptEnabled = 1;
				console.log("interupts are on")
				break;
			}
			
			case 0xfc: {
				if (this.debugMode) debugString += `CM addr, if S: ${this.S} do push pc and change PC: 0x${this.PC.toString(16)} to addr: 0x${this.inst.addr.toString(16)}`;
				if (this.S === 1) {
					this.call(this.inst.addr)
					this.cycles += 6;
				}
				break;
			}
			
			case 0xfe: {
				const data = this.inst.byte2;
				const res = new Uint8Array([this.A - data])[0];
				if (this.debugMode) debugString += `CPI D8, do (A: 0x${this.A.toString(16)} - byte2: 0x${this.inst.byte2.toString(16)}): 0x${res.toString(16)} and set all flags `;
				
				if (res === 0) {
					this.Z = 1;
					if (this.debugMode) debugString += `Z flag is set`;
				} else {
					this.Z = 0;
					if (this.debugMode) debugString += `Z flag is unset`;
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (res >> 7 & 1) {
					this.S = 1;
					if (this.debugMode) debugString += "S flag is set";
				} else {
					this.S = 0;
					if (this.debugMode) debugString += "S flag is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if (this.A < data) {
					this.CY = 1;
					if (this.debugMode) debugString += "CY is set";
				} else {
					this.CY = 0;
					if (this.debugMode) debugString += "CY is unset";
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				const parity = this.parity(res);
				if (parity) {
					this.P = 1;
					if (this.debugMode) debugString += "P flag is set"
				} else {
					this.P = 0;
					if (this.debugMode) debugString += "P flag is unset"
				}
				
				if (this.debugMode) debugString += " =---= ";
				
				if ((this.A & 0x0f) < (this.inst.byte2 & 0x0f)) {
					this.AC = 1;
					if (this.debugMode) debugString += "AC flag is set";
				} else {
					this.AC = 0;
					if (this.debugMode) debugString += "AC flag is unset";
				}
				break;
			}
			
			default: {
				console.log(`not implemented opcode: 0x${this.inst.opcode.toString(16)}`);
				this.status = 0;
			}
		}
		if (this.debugMode) {
			console.log(debugString);
			this.LOG += debugString + "\n";
		}
		
	}
	
}

class Instruction {
	constructor() {
		this.opcode;
		this.byte2;
		this.byte3;
		this.addr;
		this.size;
	}
	setSize() {
		this.size = OPCODE_SIZES[this.opcode];
		if (!this.size) {
			console.log(`this opcodes: 0x${this.opcode.toString(16)}, size is not in the OPCODE_SIZES`);
			return 0;
		}
		return 1;
	}
}

export default Machine;